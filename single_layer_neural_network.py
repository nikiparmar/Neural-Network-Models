	...
	def set_weights(self, weights):
		if self.n_hidden == 0:
			...
			for i in range(0, self.n_layers-3):
				self.weights += [np.array(weights[start:start+(n_hidden*n_hidden)]).reshape(self.n_hidden, self.n_hidden)]
				start += (n_hidden*n_hidden)
                        self.weights += [np.array(weights[(self.n_input)*self.n_hidden:]).reshape(self.n_hidden, self.n_output)]

	def set_bias(self, bias):
		self.bias = [np.array(bias[:self.n_hidden]).reshape(1, self.n_hidden)]
		start = self.n_hidden
		for i in range(0, self.n_layers-3):
			self.bias += self.bias = [np.array(bias[start:start+self.n_hidden]).reshape(1, self.n_hidden)]
			start += self.n_hidden
